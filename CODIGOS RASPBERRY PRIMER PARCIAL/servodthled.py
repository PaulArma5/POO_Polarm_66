...
def handle(msg):
    ...
    if command == '/start':
        ...
        led_status = True
        bot.sendMessage(chat_id, 'LED encendido y parpadeando.')
        ...
        mover_servo(0)
        bot.sendMessage(chat_id, 'Servo movido a 0°.')
    elif command == '/cerrar':
        mover_servo(180)
        bot.sendMessage(chat_id, 'Servo movido a 180°.')
    else:
        bot.sendMessage(chat_id, 'Comando no reconocido. Usa /start para ayuda.')
